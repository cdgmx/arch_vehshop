...
AddEventHandler('AnnounceConcessOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Car Dealer', '~b~Ad', 'The Car dealer has just opened! Come buy your vehicles!', 'CHAR_CARSITE', 8)
	end
end)
...
AddEventHandler('AnnounceConcessFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Car Dealer', '~b~Ad', 'The Car dealer has just closed, pass later.', 'CHAR_CARSITE', 8)
	end
end)
